...
use crate::encryption::{
    decrypt_data_file_with_pin, derive_carrier_key_from_pin, find_pdvrdt_iccp_payload,
    has_pdvrdt_profile_markers, minimum_stream_cipher_size, ProfileOffsets, DEFAULT_OFFSETS,
    MASTODON_OFFSETS, MAX_MASTODON_PROFILE_BYTES, PDVRDT_IDAT_PREFIX,
};
...
fn locate_metadata_embedded_data(png_vec: &[u8]) -> Result<Option<EmbeddedProfile>> {
...
pub fn recover_data(png_vec: &mut Vec<u8>) -> Result<()> {
    let outcome = (|| -> Result<()> {
        ...
        if let Some(embedded) = locate_metadata_embedded_data(png_vec)? {
            ...
            // The Reddit carrier's sample positions are derived from the PIN, so
            // there is no way to answer "is there a payload here?" before asking
            // for it. That is the property being bought: a wrong PIN and an
            // ordinary PNG are indistinguishable to anyone but the holder, so
            // they share one message below.
            get_pin(&mut recovery_pin)?;

            let carrier_key = derive_carrier_key_from_pin(&recovery_pin)?;
            match extract_reddit_png_payload(png_vec, carrier_key)? {
...
mod tests {
    ...
    fn locator_rejects_malformed_png_structure() {
        ...
        trailing.push(0);
        assert!(locate_metadata_embedded_data(&trailing)
            .unwrap_err()
            .to_string()
    ...
    fn locator_rejects_duplicate_iccp_and_embedded_payloads() {
        ...
        assert!(locate_metadata_embedded_data(&duplicate_iccp)
            .unwrap_err()
            .to_string()
        ...
        assert!(locate_metadata_embedded_data(&duplicate_payload)
            .unwrap_err()
            .to_string()
