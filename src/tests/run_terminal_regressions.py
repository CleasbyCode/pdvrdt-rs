#!/usr/bin/env python3
"""Linux PTY regressions for PIN masking and terminal restoration.

Uses the C++ golden fixtures from --fixtures (default: ../src/tests).
Rust preserves its runtime SIGPIPE disposition: broken output returns an error
instead of terminating by SIGPIPE, and no stdout write may panic.
"""
...
def test_signal(binary: Path, image: Path, signal_number: int):
    ...
    try:
        ...
        return_code = process.wait(timeout=5.0)
        expected_code = 1 if signal_number == signal.SIGPIPE else -signal_number
        if return_code != expected_code:
            raise AssertionError(
                f"signal {signal_number}: expected {expected_code}, got {return_code}")
...
def test_masking(binary: Path, image: Path):
    ...
    try:
        ...
        if any(marker in output for marker in
               (b"AddressSanitizer", b"LeakSanitizer", b"runtime error:", b"panicked at")):
...
def test_broken_stdout(binary: Path, image: Path, stage: str):
    ...
    try:
        if stage == "prompt":
            ...
            # before exec so an output error is guaranteed at that write.
            os.close(read_fd)
            read_fd = -1
        process = subprocess.Popen(
            [str(binary), "recover", str(image)],
        ...
        if return_code != 1:
            raise AssertionError(
                f"broken stdout at {stage}: expected error exit 1, got {return_code}")
        ...
        if b"panicked at" in output or b"PIN Error:" not in output:
            raise AssertionError(f"broken stdout at {stage}: expected checked PIN error: {output!r}")
    finally:
...
def main() -> int:
    ...
    binary = Path(args.bin).resolve() if args.bin else root / "target/release/pdvrdt-rs"
    fixtures = Path(args.fixtures).resolve() if args.fixtures else root.parent / "src/tests"
    image = fixtures / "golden/default_text/embedded.png"
    if not os.access(binary, os.X_OK):
        parser.error(f"binary is not executable: {binary}")
    if not image.is_file():
        parser.error(f"golden PNG fixture is missing: {image}")
    ...
    for stage in ("prompt", "digit", "backspace", "newline"):
        ...
        print(f"[PASS] terminal restored after broken stdout at PIN {stage}")
    return 0
