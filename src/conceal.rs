...
use crate::encryption::{encrypt_compressed_file_to_profile, MAX_MASTODON_PROFILE_BYTES};
use crate::file_utils::{
    close_file_or_throw, has_file_extension, has_safe_embedded_filename, open_input_file,
...
fn size_limit_for_option(option: PlatformOption) -> (usize, &'static str) {
    ...
    // Hard encode limit (20 MiB). Hosting sites often advertise ~19 MB.
    ...
    match option {
        ...
        PlatformOption::Reddit => (MAX_SIZE_REDDIT, "Reddit"),
...
const REDDIT_PADDING_CHUNK_TOTAL_SIZE: usize = 0x80000 + PNG_CHUNK_OVERHEAD;

fn validate_data_filename(data_file_path: &Path, data_filename: &[u8]) -> Result<()> {
    const FILENAME_MAX_LEN: usize = 20;

    if !has_safe_embedded_filename(data_file_path) {
        bail!("Data File Error: Embedded filename is unsafe. Filenames may not begin with '.' or '-'.");
    }
...
fn maximum_profile_size_for_encryption(
    ...
    if option == PlatformOption::Mastodon {
        return Ok(MAX_MASTODON_PROFILE_BYTES);
    }

    let (output_limit, label) = size_limit_for_option(option);
    let mut fixed_output_size = checked_add_size(
        optimized_png_size,
        PNG_CHUNK_OVERHEAD + DEFAULT_IDAT_PREFIX_BYTES,
        "File Size Error: Final output size overflow.",
    )?;
    if option == PlatformOption::Reddit {
        fixed_output_size = checked_add_size(
            fixed_output_size,
...
fn maximum_mastodon_compressed_profile_size(optimized_png_size: usize) -> Result<usize> {
    let (output_limit, label) = size_limit_for_option(PlatformOption::Mastodon);
    let fixed_output_size = checked_add_size(
        optimized_png_size,
        PNG_CHUNK_OVERHEAD + MASTODON_ICCP_PREFIX_BYTES,
        "File Size Error: Final output size overflow.",
    )?;
    if fixed_output_size >= output_limit {
        bail!(
...
pub fn conceal_data(
    ...
    if is_mastodon {
        ...
        let compressed_profile = deflate_mastodon_profile(
            &profile_vec,
            option,
            is_compressed,
            max_compressed_profile_size,
        )?;
        ...
        let mut output_size = checked_add_size(
            png_vec.len(),
