use crate::compression::zlib_inflate_prefix;
use crate::encryption::{
    has_supported_kdf_metadata_at, DEFAULT_OFFSETS, DEFAULT_PDV_SIG_OFFSET, MASTODON_OFFSETS,
    MASTODON_PDV_SIG_OFFSET, PDVRDT_SIG,
};
...
const PDVRDT_IDAT_PREFIX: &[u8; 3] = b"\x78\x5e\x5c";
const PDVRDT_ICCP_PREFIX: &[u8; 5] = b"icc\0\0";
...
fn preflight_png(png: &[u8]) -> Result<PngPreflight> {
    ...
    let row_bytes = checked_add(
        row_bits / 8,
        usize::from(row_bits % 8 != 0),
        "PNG Error: Scanline size overflow.",
    )?;
    let filtered_row_bytes = checked_add(row_bytes, 1, "PNG Error: Scanline size overflow.")?;
    let inflated_size = checked_mul(
        filtered_row_bytes,
        height_usize,
...
fn bytes_equal_at(data: &[u8], offset: usize, expected: &[u8]) -> bool {
    offset <= data.len()
        && expected.len() <= data.len() - offset
...
fn looks_like_pdvrdt_idat(chunk_data: &[u8]) -> bool {
    ...
    let profile = &chunk_data[PDVRDT_IDAT_PREFIX.len()..];
    has_supported_kdf_metadata_at(profile, DEFAULT_OFFSETS.kdf_metadata)
        && bytes_equal_at(profile, DEFAULT_PDV_SIG_OFFSET, PDVRDT_SIG)
}

fn looks_like_pdvrdt_iccp(chunk_data: &[u8]) -> bool {
    if !chunk_data.starts_with(PDVRDT_ICCP_PREFIX) {
        return false;
    }
    let compressed = &chunk_data[PDVRDT_ICCP_PREFIX.len()..];
    if compressed.is_empty() {
        return false;
    }

    let Some(prefix_size) = MASTODON_PDV_SIG_OFFSET.checked_add(PDVRDT_SIG.len()) else {
        return false;
    };
    let Ok(profile_prefix) = zlib_inflate_prefix(compressed, prefix_size) else {
        return false;
    };
    profile_prefix.len() == prefix_size
        && has_supported_kdf_metadata_at(&profile_prefix, MASTODON_OFFSETS.kdf_metadata)
        && bytes_equal_at(&profile_prefix, MASTODON_PDV_SIG_OFFSET, PDVRDT_SIG)
}
...
fn collect_color_metadata(png: &[u8], palette_from_rgba: bool) -> Result<Vec<u8>> {
    ...
    while pos < png.len() {
        ...
        if is_color_metadata(chunk.kind)
            && !(chunk.kind == TYPE_ICCP && looks_like_pdvrdt_iccp(chunk.data))
        {
...
fn strip_and_copy_chunks(image_file_vec: &mut Vec<u8>, color_type: u8) -> Result<()> {
    ...
    let first_chunk_pos = ihdr.offset + ihdr.total_size;
    compact_chunks_after_ihdr(image_file_vec, first_chunk_pos, |kind, data| {
        (kind == TYPE_PLTE && color_type == INDEXED_PLTE)
            || kind == TYPE_TRNS
            || (is_color_metadata(kind) && !(kind == TYPE_ICCP && looks_like_pdvrdt_iccp(data)))
            || (kind == TYPE_IDAT && !looks_like_pdvrdt_idat(data))
            || kind == TYPE_IEND
    })
}
...
mod tests {
    use super::*;
    use crate::encryption::{
        KDF_ALG_ARGON2ID13, KDF_ALG_OFFSET, KDF_SENTINEL, KDF_SENTINEL_OFFSET,
    };
    ...
    fn exact_default_payload_detection_requires_fixed_markers() {
        let mut profile = vec![0u8; DEFAULT_PDV_SIG_OFFSET + PDVRDT_SIG.len()];
        mark_supported_profile(
            &mut profile,
            DEFAULT_OFFSETS.kdf_metadata,
            DEFAULT_PDV_SIG_OFFSET,
        );
    ...
    fn exact_mastodon_payload_detection_uses_bounded_prefix() {
        let mut profile = vec![0u8; MASTODON_PDV_SIG_OFFSET + PDVRDT_SIG.len()];
        mark_supported_profile(
            &mut profile,
            MASTODON_OFFSETS.kdf_metadata,
            MASTODON_PDV_SIG_OFFSET,
        );
