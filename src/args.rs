use crate::common::{
    Mode, PlatformOption, MAX_COVER_IMAGE_SIZE, MAX_REDDIT_COVER_IMAGE_SIZE,
    REDDIT_UPLOAD_SIZE_LIMIT,
};
use crate::image::{MAX_COVER_DIMENSION, MAX_REDDIT_COVER_DIMENSION};
...
const _: () = assert!(
    MAX_COVER_IMAGE_SIZE == 4 * 1024 * 1024,
    "--info quotes a 4 MiB cover-image limit; keep it in step with MAX_COVER_IMAGE_SIZE."
);
const _: () = assert!(
    MAX_REDDIT_COVER_IMAGE_SIZE == 16 * 1024 * 1024,
    "--info quotes a 16 MiB Reddit cover-image limit; keep it in step with MAX_REDDIT_COVER_IMAGE_SIZE."
);
const _: () = assert!(
...
const _: () = assert!(
    ...
    "--info quotes a 4096x4096 cover-dimension limit; keep it in step with MAX_COVER_DIMENSION."
);
const _: () = assert!(
    MAX_REDDIT_COVER_DIMENSION == 8192,
    "--info quotes an 8192x8192 Reddit cover-dimension limit; keep it in step with MAX_REDDIT_COVER_DIMENSION."
);
...
Cover image limits: in the default and Mastodon modes the PNG cover must be
4 MiB or smaller, and neither side may exceed 4096 pixels. Reddit mode instead
permits a cover up to 16 MiB and 8192x8192, because its capacity comes from the
pixels themselves.

Payload limits: Reddit mode limits its payload input to 20 MiB, the same ceiling
the finished image must meet. Other modes have no separate payload-input limit;
their output limit applies after compression.
...
after compression, so a larger, highly compressible input can still succeed.
	...
	• Reddit    (16 MiB cover, 20 MiB payload and output) — (use -r option).
	• ImgPile   (8 MB)
...
──────────────────────────
  ...
  -r (Reddit) : Creates an adaptive spatial "file-embedded" RGB PNG for Reddit.
      ...
      Carrier positions are derived from the PIN using Argon2id to make PIN
      guessing expensive. Recovery reports the same error for a wrong PIN and
      an image with no matching carrier.

      New Reddit images require pdvrdt 5.1 or later to recover. Existing 5.0
      images remain recoverable; recreate them to gain the stronger PIN protection.

      A Reddit cover image must be no larger than 16 MiB and 8192x8192 pixels;
      the payload input file must be no larger than 20 MiB, as must the finished
      image. The payload is compressed before its carrier-capacity check.
...
• Default and Mastodon modes carry the payload in a PNG chunk. That a chunk is present is visible to
  anyone; what is hidden is its contents. Reddit mode derives its carrier positions from the
  recovery PIN. Keyed positions alone do not guarantee that an image is undetectable as steganography.
