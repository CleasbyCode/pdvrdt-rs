use crate::common::{
    ...
    MAX_REDDIT_COVER_IMAGE_SIZE, PLATFORM_LIMITS, REDDIT_UPLOAD_SIZE_LIMIT,
};
use crate::compression::{zlib_deflate_file, zlib_store_span};
use crate::crypto::randombytes_uniform;
use crate::encryption::{
    compute_reddit_encrypted_profile_size, derive_carrier_key_from_pin,
    encrypt_compressed_file_to_profile, encrypt_prepared_data_to_profile,
...
use crate::image::{
    optimize_image, prepare_cover_for_reddit_carrier, prepare_image_for_mastodon_embedding,
};
...
fn validate_reddit_preliminary_limits(cover_size: usize, payload_size: usize) -> Result<()> {
    // The cover is held to the tighter cover-image limit; the payload only has to
    // clear Reddit's upload ceiling, which is also what the finished image must
    // satisfy. Two different numbers, so each is quoted from its own constant.
    const COVER_LIMIT_MIB: usize = MAX_REDDIT_COVER_IMAGE_SIZE / (1024 * 1024);
    const PAYLOAD_LIMIT_MIB: usize = REDDIT_UPLOAD_SIZE_LIMIT / (1024 * 1024);

    let cover_too_large = cover_size > MAX_REDDIT_COVER_IMAGE_SIZE;
    let payload_too_large = payload_size > REDDIT_UPLOAD_SIZE_LIMIT;
    if cover_too_large && payload_too_large {
        bail!(
            "File Size Error: Cover image exceeds the {COVER_LIMIT_MIB} MiB Reddit cover image size limit, \
and payload file exceeds Reddit's {PAYLOAD_LIMIT_MIB} MiB upload size limit."
        );
    }
    if cover_too_large {
        bail!("Image File Size Error: Cover image exceeds the {COVER_LIMIT_MIB} MiB Reddit cover image size limit.");
    }
    if payload_too_large {
        bail!("Data File Size Error: Payload file exceeds Reddit's {PAYLOAD_LIMIT_MIB} MiB upload size limit.");
    }
    Ok(())
}
...
fn prepare_reddit_carrier(png_vec: &mut Vec<u8>) -> Result<RedditPngCarrier> {
    // Validation only -- not optimize_image(). The Reddit carrier decodes the
    ...
    prepare_cover_for_reddit_carrier(png_vec)?;
    let carrier = prepare_reddit_png_carrier(png_vec)?;
    *png_vec = Vec::new();
    Ok(carrier)
}
...
struct RedditCapacityLimits {
    conservative_compressed: usize,
    recommended_compressed: usize,
}

fn max_reddit_compressed_payload(carrier_capacity: usize, filename_length: usize) -> Result<usize> {
    let fits = |compressed_size| -> Result<bool> {
        Ok(
            compute_reddit_encrypted_profile_size(compressed_size, filename_length)?
                <= carrier_capacity,
        )
    };
    if !fits(0)? {
    ...
    let mut upper = carrier_capacity;
    while lower < upper {
...
fn reddit_capacity_limits(carrier: &RedditPngCarrier) -> Result<RedditCapacityLimits> {
    let conservative_compressed =
        max_reddit_compressed_payload(carrier.payload_capacity, DATA_FILENAME_MAX_LENGTH)?;
    Ok(RedditCapacityLimits {
        conservative_compressed,
        recommended_compressed: conservative_compressed.saturating_sub(BYTES_PER_KIBIBYTE),
    })
}

fn reddit_cover_summary(carrier: &RedditPngCarrier, source_cover_size: usize) -> String {
    format!(
        "Cover Image: {}KiB, {}x{}, Non-interlaced 8-bit RGB PNG, Adaptive (1,15,4) matrix embedding.\n\n",
...
fn reddit_capacity_limits_report(
    carrier: &RedditPngCarrier,
    limits: RedditCapacityLimits,
) -> String {
    const CAPACITY_LABEL_WIDTH: usize = 71;
    format!(
        "{:<width$}{} bytes (~{}KiB).\n\
         {:<width$}{} bytes (~{}KiB).\n\
         {:<width$}{} bytes (~{}KiB).\n",
        ...
        carrier.payload_capacity / BYTES_PER_KIBIBYTE,
        ...
        width = CAPACITY_LABEL_WIDTH
    )
}

fn report_reddit_capacity_failure(
    carrier: &RedditPngCarrier,
    source_cover_size: usize,
    compressed_size: usize,
) -> Result<()> {
    let limits = reddit_capacity_limits(carrier)?;
    let report = format!(
        "\n{}Compressed data file (payload) size: {} bytes ({}KiB).\n\n{}",
        reddit_cover_summary(carrier, source_cover_size),
        compressed_size,
        compressed_size / BYTES_PER_KIBIBYTE,
        reddit_capacity_limits_report(carrier, limits)
    );
    write_stdout_checked(report.as_bytes())?;
    bail!(
        "Data File Size Error: \n\nAlready-compressed payload file size of {} bytes ({}KiB) exceeds the recommended maximum limit of {} bytes (~{}KiB) for this cover image.",
        compressed_size,
        compressed_size / BYTES_PER_KIBIBYTE,
        limits.recommended_compressed,
        limits.recommended_compressed / BYTES_PER_KIBIBYTE
...
fn conceal_reddit_data(
    ...
    let source_cover_size = png_vec.len();
    let mut carrier = prepare_reddit_carrier(png_vec)?;
    ...
    let encrypted_profile_size =
        compute_reddit_encrypted_profile_size(compressed.len(), data_filename.len())?;
    if encrypted_profile_size > carrier.payload_capacity {
        return report_reddit_capacity_failure(&carrier, source_cover_size, compressed.len());
    }
    ...
    if profile_vec.len() != encrypted_profile_size {
        bail!("Internal Error: Reddit encrypted profile size accounting mismatch.");
    }

    // Carrier v2 uses Argon2id13 so testing these sample positions and header
    // magic does not provide a cheap PIN oracle alongside payload encryption.
...
pub fn display_reddit_capacity(png_vec: &mut Vec<u8>) -> Result<()> {
    let source_cover_size = png_vec.len();
    let carrier = prepare_reddit_carrier(png_vec)?;
    let limits = reddit_capacity_limits(&carrier)?;
    let minimum_single_frame_overhead = compute_reddit_encrypted_profile_size(1, 1)? - 1;
    let maximum_single_frame_overhead =
        compute_reddit_encrypted_profile_size(1, DATA_FILENAME_MAX_LENGTH)? - 1;
    ...
    let report = format!(
        "\nReddit capacity check for conceal -r mode only.\n\n{}{}\nImportant:\n\n\
         ...
         The final embedded PNG size can differ from the cover image and must remain within 20MiB.\n\n",
        reddit_cover_summary(&carrier, source_cover_size),
        reddit_capacity_limits_report(&carrier, limits),
...
mod tests {
    ...
    #[test]
    fn reddit_capacity_limit_includes_envelope_and_recommended_margin() {
        let conservative = max_reddit_compressed_payload(1619, DATA_FILENAME_MAX_LENGTH).unwrap();
        assert_eq!(conservative, 1401);
        assert_eq!(conservative.saturating_sub(BYTES_PER_KIBIBYTE), 377);
    }
}
