...
use crate::compression::{zlib_inflate_prefix, zlib_inflate_span_bounded, zlib_inflate_to_file};
use crate::crypto::randombytes_uniform;
use crate::crypto::secretstream;
use crate::encryption::{
    decrypt_data_file, has_supported_kdf_metadata_at, ProfileOffsets, DEFAULT_OFFSETS,
    DEFAULT_PDV_SIG_OFFSET, MASTODON_OFFSETS, MASTODON_PDV_SIG_OFFSET, MAX_MASTODON_PROFILE_BYTES,
    PDVRDT_SIG,
};
use crate::file_utils::{close_file_or_throw, has_safe_embedded_filename, open_write_new_nofollow};
...
fn bytes_equal_at(data: &[u8], offset: usize, expected: &[u8]) -> bool {
    span_has_range(data, offset, expected.len())
        && data[offset..offset + expected.len()] == *expected
}

fn has_recoverable_kdf_metadata(profile: &[u8], offsets: &ProfileOffsets) -> bool {
    let min_stream_ciphertext_size = secretstream::HEADERBYTES + 4 + secretstream::ABYTES;
    has_supported_kdf_metadata_at(profile, offsets.kdf_metadata)
        && span_has_range(profile, offsets.encrypted_file, min_stream_ciphertext_size)
}
...
fn try_locate_default_profile_in_idat(
    ...
    if !has_recoverable_kdf_metadata(profile, &DEFAULT_OFFSETS)
        || !bytes_equal_at(profile, DEFAULT_PDV_SIG_OFFSET, PDVRDT_SIG)
    {
        return None;
    }
    Some((data_index + IDAT_PREFIX.len(), profile.len()))
}
...
fn try_extract_mastodon_profile_from_iccp(iccp_data: &[u8]) -> Result<Option<Vec<u8>>> {
    ...
    const PROFILE_PREFIX_SIZE: usize = MASTODON_PDV_SIG_OFFSET + PDVRDT_SIG.len();
    ...
    let profile_prefix = match zlib_inflate_prefix(compressed_profile, PROFILE_PREFIX_SIZE) {
        Ok(prefix) => prefix,
    ...
    if !has_recoverable_kdf_metadata(&profile, &MASTODON_OFFSETS)
        || !bytes_equal_at(&profile, MASTODON_PDV_SIG_OFFSET, PDVRDT_SIG)
    {
        return Ok(None);
    }
    Ok(Some(profile))
...
mod tests {
    ...
    fn default_payload_chunk() -> Vec<u8> {
        let min_ciphertext = secretstream::HEADERBYTES + 4 + secretstream::ABYTES;
        ...
        profile[DEFAULT_PDV_SIG_OFFSET..DEFAULT_PDV_SIG_OFFSET + PDVRDT_SIG.len()]
            .copy_from_slice(PDVRDT_SIG);
