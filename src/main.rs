...
use pdvrdt::common::{FileTypeCheck, Mode, PlatformOption};
use pdvrdt::conceal;
...
fn run() -> anyhow::Result<()> {
    ...
    let file_type = match (program_args.mode, program_args.option) {
        (Mode::Recover, _) => FileTypeCheck::EmbeddedImage,
        (Mode::Capsize, _) => FileTypeCheck::RedditCoverImage,
        (Mode::Conceal, PlatformOption::Reddit) => FileTypeCheck::RedditCoverImage,
        (Mode::Conceal, _) => FileTypeCheck::CoverImage,
    };
    ...
    match program_args.mode {
        ...
        Mode::Capsize => {
            conceal::display_reddit_capacity(&mut png_vec)?;
        }
    }
