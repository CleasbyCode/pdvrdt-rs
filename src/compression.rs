...
///
/// `Compression::default()` is level 6, not 9: level 9 costs substantially more
/// time for only a marginal ratio improvement. Matches the C++ implementation.
fn payload_deflate_level(is_compressed_file: bool) -> Compression {
...
where
    F: FnMut(&[u8]) -> Result<()>,
{
    let mut encoder = ZlibEncoder::new(ChunkWriter::new(on_chunk), Compression::none());
    ...
    F: FnMut(&[u8]) -> Result<()>,
{
    ...
    let mut encoder = ZlibEncoder::new(
        ...
        payload_deflate_level(is_compressed_file),
    );
...
mod tests {
    ...
    fn output_exactly_at_the_bound_is_accepted() {
        ...
        assert_eq!(
            zlib_inflate_span_bounded(&compressed, payload.len()).unwrap().len(),
            payload.len()
        );
    ...
    fn stored_span_is_a_valid_zlib_stream_that_does_not_deflate() {
        ...
        let ciphertext: Vec<u8> = (0..8192u32).map(|i| (i.wrapping_mul(2654435761) >> 16) as u8).collect();
        ...
        assert_eq!((stored[2] >> 1) & 3, 0, "first deflate block must be stored");
        assert_eq!(
