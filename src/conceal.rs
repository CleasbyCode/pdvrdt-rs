...
fn report_reddit_capacity_failure(
    ...
    // The limit conceal_reddit_data actually enforced is the one for *this*
    // payload's filename, which is usually shorter than the 20-character worst
    // case the summary above quotes -- and it is at least a full 1 KiB above the
    ...
    let enforced_maximum =
        max_reddit_compressed_payload(carrier.payload_capacity, filename_length)?;
    let report = format!(
    ...
    write_stdout_checked(report.as_bytes())?;
    bail!(
        "Data File Size Error: \n\nCompressed payload size of {} bytes ({}KiB) exceeds this cover image's maximum of {} bytes (~{}KiB) for a {}-character filename.\nWhere capacity permits, leave at least 1KiB of headroom below that ({} bytes).",
        compressed_size,
        compressed_size / BYTES_PER_KIBIBYTE,
        enforced_maximum,
        enforced_maximum / BYTES_PER_KIBIBYTE,
        filename_length,
        limits.recommended_compressed
    )
}
...
fn conceal_reddit_data(
    ...
    if encrypted_profile_size > carrier.payload_capacity {
        return report_reddit_capacity_failure(
            &carrier,
            source_cover_size,
            compressed.len(),
            data_filename.len(),
