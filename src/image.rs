...
/// larger bound.
pub const MAX_COVER_DIMENSION: u32 = 4096;
pub const MAX_REDDIT_COVER_DIMENSION: u32 = 8192;
...
fn require_valid_color_metadata_chunk(
    chunk: &PngChunk,
    color_type: u8,
    bit_depth: u8,
) -> Result<()> {
    ...
    match chunk.kind {
        ...
        TYPE_SBIT => {
            ...
            let sample_bits = if color_type == INDEXED_PLTE {
                8
            } else {
                bit_depth
            };
...
/// Validate the data that the pixel decoder will consume, before its allocations.
///
/// `png` 0.17's IHDR-derived inflation limit is only an allocation hint: it is
/// discarded when the stream exceeds it. Its general byte limit does not cap
/// inflation either. A bounded streaming pass enforces the exact scanline size,
/// including packed samples and Adam7, and validates palettes the crate accepts
/// without checking their lengths. The cover repair path uses this same pass.
pub(crate) fn require_valid_png_decode(png: &[u8]) -> Result<()> {
    let preflight = geometry_within_decode_limit(png)?;
    inspect_idat_stream(png, &preflight).map(|_| ())
}

fn require_valid_palette(chunk: &PngChunk<'_>, color_type: u8, bit_depth: u8) -> Result<()> {
    let length = chunk.data.len();
    if !(3..=768).contains(&length)
        || !length.is_multiple_of(3)
        || (color_type == INDEXED_PLTE && length / 3 > (1usize << bit_depth))
    {
        bail!("PNG Error: invalid PLTE size.");
    }
    Ok(())
}
...
/// Cover-image dimension policy for the mode in use. Read straight from IHDR so
/// an oversized cover is named as such before anything else weighs in: the same
/// image would otherwise trip the decoder's byte ceiling first and be reported as
/// a generic safety-limit failure, which says nothing about what to do about it.
fn require_cover_dimensions_within_limit(png: &[u8], max_dimension: u32) -> Result<()> {
    let ihdr = read_required_ihdr(png)?;
    ...
    if width > max_dimension || height > max_dimension {
        bail!(
            "Image File Error: Cover image dimensions of {width}x{height} exceed the maximum of {max_dimension}x{max_dimension} for this conceal mode."
        );
    }
    Ok(())
}

fn preflight_png(png: &[u8], max_cover_dimension: u32) -> Result<PngPreflight> {
    ...
    // Mode policy first, so it owns the error message for an oversized cover.
    require_cover_dimensions_within_limit(png, max_cover_dimension)?;

    let ihdr = read_required_ihdr(png)?;
    // read_required_ihdr has already fixed the length at IHDR_DATA_SIZE.
    let geometry = geometry_within_decode_limit(png)?;
    let ihdr_bit_depth = geometry.bit_depth;
    let ihdr_color_type = geometry.color_type;
    ...
    while pos < png.len() {
        ...
        if chunk.kind == TYPE_PLTE {
            require_valid_palette(&chunk, ihdr_color_type, ihdr_bit_depth)?;
        }
...
struct IdatStreamInfo {
    consumed: usize,
    total: usize,
}

/// Inspect concatenated IDAT data without retaining its decompressed contents.
/// At most one byte beyond IHDR's exact filtered size is ever produced: the
/// sentinel detects overflow while allowing an exact-size stream's trailer to
/// finish. Bytes after a complete, correctly sized stream are repairable; a
/// truncated, short, or overexpanded stream is not.
fn inspect_idat_stream(png: &[u8], preflight: &PngPreflight) -> Result<IdatStreamInfo> {
    let expected = preflight.inflated_size;
    let mut decompress = flate2::Decompress::new(true);
    let mut scratch = vec![0u8; (expected + 1).min(64 * 1024)];
    ...
    while pos < png.len() {
        ...
        if chunk.kind == TYPE_PLTE {
            require_valid_palette(&chunk, preflight.color_type, preflight.bit_depth)?;
        }
        if chunk.kind == TYPE_IDAT {
            idat_total = checked_add(
                idat_total,
                chunk.data.len(),
                "PNG Error: IDAT size overflow.",
            )?;
            let mut offset = 0usize;
            while !stream_ended {
                let consumed_before = decompress.total_in();
                let produced_before = decompress.total_out();
                let output_space = scratch.len().min(expected - produced_before as usize + 1);
                let status = decompress
                    .decompress(
                        &chunk.data[offset..],
                        &mut scratch[..output_space],
                        flate2::FlushDecompress::None,
                    )
                    .map_err(|_| anyhow!("PNG decode error: corrupt image data stream."))?;
                if decompress.total_out() > expected as u64 {
                    bail!("PNG decode error: inflated image exceeds IHDR scanline size.");
                }
                ...
                if status == flate2::Status::StreamEnd {
                    ...
                    if offset < chunk.data.len() {
                        bail!("PNG decode error: image data stream made no progress.");
                    }
                    break;
                } else if offset == chunk.data.len() && produced < output_space {
                    // A full output buffer may leave pending output even when
                    // this chunk's input is exhausted. Drain it before waiting
                    // for the next IDAT, preserving a split zlib trailer.
                    break;
                }
            }
        }
    ...
    if !stream_ended {
        bail!("PNG decode error: incomplete image data stream.");
    }
    if decompress.total_out() != expected as u64 {
        bail!("PNG decode error: inflated image does not match IHDR scanline size.");
    }
    Ok(IdatStreamInfo {
...
/// Drop bytes after the exact-size PNG zlib stream so cover reuse does not copy
/// foreign data into a new output. The bounded inspection also protects the
/// pixel decoder from streams whose output disagrees with their IHDR geometry.
fn strip_trailing_idat_data(png: &mut Vec<u8>) -> Result<bool> {
    let preflight = geometry_within_decode_limit(png)?;
    let stream = inspect_idat_stream(png, &preflight)?;
    let consumed_total = stream.consumed;
    if consumed_total == stream.total {
        return Ok(false);
    }
    let ihdr = read_required_ihdr(png)?;
...
pub fn prepare_cover_for_reddit_carrier(image_file_vec: &mut Vec<u8>) -> Result<()> {
    preflight_png(image_file_vec, MAX_REDDIT_COVER_DIMENSION)?;
...
pub fn optimize_image(image_file_vec: &mut Vec<u8>) -> Result<ImageCheckResult> {
    ...
    let preflight = preflight_png(image_file_vec, MAX_COVER_DIMENSION)?;
    remove_existing_pdvrdt_payload_chunks(image_file_vec)?;
...
mod tests {
    ...
    fn structural_png(width: u32, height: u32, extra_chunks: &[Vec<u8>]) -> Vec<u8> {
        structural_png_typed(width, height, 8, TRUECOLOR_RGB, extra_chunks)
    }

    fn structural_png_typed(
        width: u32,
        height: u32,
        bit_depth: u8,
        color_type: u8,
        ...
        png.extend_from_slice(
            &make_png_chunk(TYPE_IHDR, &ihdr(width, height, bit_depth, color_type)).unwrap(),
        );
    ...
    fn compressed_scanlines(scanlines: &[u8]) -> Vec<u8> {
    ...
    fn png_with_idat(
        ...
        let mut png = PNG_SIGNATURE.to_vec();
        png.extend_from_slice(&make_png_chunk(TYPE_IHDR, header).unwrap());
        if let Some(palette) = palette {
            png.extend_from_slice(&make_png_chunk(TYPE_PLTE, palette).unwrap());
        }
        png.extend_from_slice(&make_png_chunk(TYPE_IDAT, &compressed[..split_at]).unwrap());
        png.extend_from_slice(&make_png_chunk(TYPE_IDAT, &compressed[split_at..]).unwrap());
        png.extend_from_slice(&make_png_chunk(TYPE_IEND, &[]).unwrap());
        png
    }
    ...
    #[test]
    fn actual_inflation_matches_packed_and_adam7_geometry() {
        // Independently counted scanline sizes, including empty Adam7 passes
        // and per-pass byte padding, rather than reusing the production formula.
        for (width, height, depth, color, interlace, expected) in [
            (1, 1, 8, TRUECOLOR_RGBA, 0, 5),
            (13, 3, 1, 0, 0, 9),
            (1, 1, 1, 0, 1, 2),
            (1, 1, 8, TRUECOLOR_RGBA, 1, 5),
            (5, 5, 1, 0, 1, 22),
            (9, 7, 8, TRUECOLOR_RGBA, 1, 266),
            (3, 2, 16, TRUECOLOR_RGB, 0, 38),
        ] {
            let mut header = ihdr(width, height, depth, color);
            header[12] = interlace;
            for count in [expected - 1, expected, expected + 1] {
                let compressed = compressed_scanlines(&vec![0; count]);
                // Every split includes boundaries inside the zlib header,
                // deflate codes, and checksum, plus empty IDAT chunks.
                for split_at in 0..=compressed.len() {
                    let png = png_with_idat(&header, &compressed, split_at, None);
                    let result = require_valid_png_decode(&png);
                    assert_eq!(result.is_ok(), count == expected,
                        "{width}x{height} depth={depth} color={color} interlace={interlace}, output={count}, split={split_at}: {result:?}");
                    if count == expected {
                        let preflight = preflight_png(&png, MAX_COVER_DIMENSION).unwrap();
                        let decoded = decode_image(&png, &preflight).unwrap();
                        assert_eq!((decoded.width, decoded.height), (width, height));
                        assert!(decoded.pixels.iter().all(|&value| value == 0));
                    }
                }
            }
        }
    ...
    #[test]
    fn tiny_ihdr_rejects_large_inflation_before_trailing_repair() {
        let mut compressed = compressed_scanlines(&vec![0; 4 * 1024 * 1024]);
        compressed.extend_from_slice(b"repairable only after a correctly sized stream");
        let png = png_with_idat(
            &ihdr(1, 1, 8, TRUECOLOR_RGBA),
            &compressed,
            compressed.len(),
            None,
        );
        let error = require_valid_png_decode(&png).unwrap_err().to_string();
        assert!(error.contains("exceeds IHDR scanline size"), "{error}");
        let mut repaired = png.clone();
        let error = strip_trailing_idat_data(&mut repaired)
            .unwrap_err()
            .to_string();
        assert!(error.contains("exceeds IHDR scanline size"), "{error}");
        assert_eq!(repaired, png);
    }

    #[test]
    fn high_compression_images_remain_valid_across_scratch_boundaries() {
        let scanlines = vec![0; 512 * (1 + 512 * 4)];
        let compressed = compressed_scanlines(&scanlines);
        assert!(compressed.len() * 100 < scanlines.len());
        for split_at in [
            0,
            compressed.len() / 2,
            compressed.len() - 1,
            ...
            let png = png_with_idat(
                &ihdr(512, 512, 8, TRUECOLOR_RGBA),
                &compressed,
                split_at,
                None,
            );
            require_valid_png_decode(&png).unwrap();
            let preflight = preflight_png(&png, MAX_COVER_DIMENSION).unwrap();
            let decoded = decode_image(&png, &preflight).unwrap();
            assert_eq!(decoded.pixels, vec![0; 512 * 512 * 4]);
            let mut copy = png.clone();
            assert!(!strip_trailing_idat_data(&mut copy).unwrap());
            assert_eq!(copy, png);
        }
    }

    #[test]
    fn bounded_inflation_rejects_truncation_and_bad_checksum() {
        let header = ihdr(1, 1, 8, TRUECOLOR_RGBA);
        let valid = compressed_scanlines(&[0; 5]);
        for length in 0..valid.len() {
            let png = png_with_idat(&header, &valid[..length], length, None);
            assert!(
                require_valid_png_decode(&png).is_err(),
                "accepted {length} bytes"
            );
        }
        let mut corrupt = valid;
        *corrupt.last_mut().unwrap() ^= 1;
        let png = png_with_idat(&header, &corrupt, corrupt.len(), None);
        assert!(require_valid_png_decode(&png).is_err());
    }

    #[test]
    fn malformed_palettes_are_rejected_before_decode_or_copy() {
        for (depth, color) in [
            (1, INDEXED_PLTE),
            (2, INDEXED_PLTE),
            (4, INDEXED_PLTE),
            ...
            let header = ihdr(1, 1, depth, color);
            let mut invalid_lengths = vec![0, 1, 2, 4, 5, 767, 769, 771];
            if color == INDEXED_PLTE {
                invalid_lengths.push(((1usize << depth) + 1) * 3);
            }
            for length in invalid_lengths {
                let png = png_with_idat(&header, &[], 0, Some(&vec![0; length]));
                for error in [
                    preflight_png(&png, MAX_COVER_DIMENSION).unwrap_err(),
                    require_valid_png_decode(&png).unwrap_err(),
                ] {
                    assert!(
                        error.to_string().contains("invalid PLTE size"),
                        "depth={depth} color={color} palette={length}: {error}"
                    );
                }
            }
        }
    ...
    #[test]
    fn valid_indexed_and_suggested_palettes_decode_at_supported_depths() {
        for (depth, color) in [
            (1, INDEXED_PLTE),
            (2, INDEXED_PLTE),
            (4, INDEXED_PLTE),
            ...
            let max_entries = if color == INDEXED_PLTE {
                1usize << depth
            } else {
                256
            };
            for entries in [1, max_entries] {
                let palette = vec![0; entries * 3];
                let sample_bytes =
                    (channels_for_color_type(color).unwrap() * depth as usize).div_ceil(8);
                let compressed = compressed_scanlines(&vec![0; 1 + sample_bytes]);
                let png = png_with_idat(
                    &ihdr(1, 1, depth, color),
                    &compressed,
                    compressed.len(),
                    Some(&palette),
                );
                require_valid_png_decode(&png).unwrap();
                let preflight = preflight_png(&png, MAX_COVER_DIMENSION).unwrap();
                decode_image(&png, &preflight).unwrap();
                let mut optimized = png;
                optimize_image(&mut optimized).unwrap();
                require_valid_png_decode(&optimized).unwrap();
            }
        }
    }
    ...
    fn trailing_idat_data_is_stripped_not_rejected() {
        ...
        // A truncated stream is rejected, never silently cut down to whatever
        // happened to inflate.
        ...
        assert!(strip_trailing_idat_data(&mut truncated).is_err());
    ...
    fn preflight_rejects_apng_and_profile_conflicts() {
        ...
        assert!(preflight_png(&apng, MAX_COVER_DIMENSION)
            .unwrap_err()
            .to_string()
        ...
        assert!(preflight_png(&conflict, MAX_COVER_DIMENSION)
            .unwrap_err()
            .to_string()
    ...
    fn preflight_bounds_dimensions_before_decode() {
        // Over the per-mode limit: the cover-dimension policy answers first, and
        // the message names the limit that was breached.
        let png = structural_png(100_000, 100_000, &[]);
        assert!(preflight_png(&png, MAX_COVER_DIMENSION)
            .unwrap_err()
            .to_string()
            .contains("exceed the maximum of 4096x4096"));

        let png = structural_png(4097, 4097, &[]);
        assert!(preflight_png(&png, MAX_COVER_DIMENSION)
            .unwrap_err()
            .to_string()
            .contains("exceed the maximum of 4096x4096"));

        // The Reddit carrier takes the same cover as far as 8192 a side.
        let png = structural_png(8192, 8192, &[]);
        assert!(preflight_png(&png, MAX_REDDIT_COVER_DIMENSION).is_ok());

        let png = structural_png(8193, 8193, &[]);
        assert!(preflight_png(&png, MAX_REDDIT_COVER_DIMENSION)
            .unwrap_err()
            .to_string()
            .contains("exceed the maximum of 8192x8192"));
    }

    #[test]
    fn preflight_still_bounds_decode_bytes_within_the_dimension_limit() {
        // 8192x8192 at 8-bit RGBA needs 268,443,648 bytes of filtered scanlines,
        // which is what MAX_DECODE_BYTES was raised past.
        let png = structural_png_typed(8192, 8192, 8, TRUECOLOR_RGBA, &[]);
        assert!(preflight_png(&png, MAX_REDDIT_COVER_DIMENSION).is_ok());

        // The same geometry at 16-bit stays rejected: ~384 MiB of scanlines.
        let png = structural_png_typed(8192, 8192, 16, TRUECOLOR_RGB, &[]);
        assert!(preflight_png(&png, MAX_REDDIT_COVER_DIMENSION)
            .unwrap_err()
            .to_string()
            .contains("Inflated image exceeds safety limit"));
    }
