...
# default payload. It also requires byte-for-byte C++/Rust Reddit capacity
# reports. Output PNG bytes are intentionally not compared because the encryption
# and codec pipelines are randomized; recovered payload bytes are.
set -euo pipefail
...
make_fixtures() {
  python3 - "$WORK/cover.png" "$WORK/cover_even_domain.png" "$WORK/p_text.txt" \
    "$WORK/p_bin.bin" "$WORK/p_stored.gz" <<'PY'
import binascii
import gzip
import struct
import sys
...
(cover_path, even_cover_path, text_path,
 binary_path, stored_path) = map(Path, sys.argv[1:])
...
def write_cover(path, width, height):
    rows = []
...
# smallest power-of-two domain containing the RGB sample count. Where that
# domain's bit length is odd the two Feistel halves differ in width, which is a
# separate code path from the equal-halves case -- and one that a change of
# carrier scheme can move independently. Both covers are therefore pinned by
# parity, not chosen for their size:
#
#   cover.png              96x96  -> 27648 samples -> 15 bits (odd)
#   cover_even_domain.png 128x128 -> 49152 samples -> 16 bits (even)
#
# The assertion below is what keeps that true: if either cover stops providing
# the geometry its case was added for, this fails loudly rather than letting the
# C++/Rust permutation agreement go untested on half its input space.
write_cover(cover_path, 96, 96)
write_cover(even_cover_path, 128, 128)

for path, want in ((cover_path, "odd"), (even_cover_path, "even")):
    ...
    if got != want:
        raise SystemExit(
            f"{path.name}: {w}x{h} gives {samples} samples, a {bits}-bit ({got}) "
            f"carrier domain, but this fixture must be {want}.")
...
check_capsize_parity() {
  local cover="$1"
  local cpp_report="$WORK/capsize-cpp.txt"
  local rust_report="$WORK/capsize-rust.txt"

  if ! "$CPP" capsize "$cover" >"$cpp_report" 2>&1; then
    echo "FAIL[capsize_cpp_rust_parity]: C++ capsize command failed" >&2
    show_log "C++ capsize output" "$cpp_report"
    FAILS=$((FAILS + 1))
    return 0
  fi
  if ! "$RS" capsize "$cover" >"$rust_report" 2>&1; then
    echo "FAIL[capsize_cpp_rust_parity]: Rust capsize command failed" >&2
    show_log "Rust capsize output" "$rust_report"
    FAILS=$((FAILS + 1))
    return 0
  fi
  if ! diff -u "$cpp_report" "$rust_report"; then
    echo "FAIL[capsize_cpp_rust_parity]: capacity reports differ" >&2
    FAILS=$((FAILS + 1))
    return 0
  fi

  echo "PASS[capsize_cpp_rust_parity]"
}
...
main() {
  ...
  # The default cover is an odd-bit-length carrier domain (unequal Feistel
  # halves). These repeat the Reddit exchange over an even one, so a change that
  # moves only one of the two geometries cannot pass unnoticed. Skipped when the
  # caller supplied their own cover, whose parity is unknown.
  if [[ -z "${COVER:-}" ]]; then
    check_capsize_parity "$WORK/cover_even_domain.png"
    run_case "$CPP" "-r" "$WORK/cover_even_domain.png" "$WORK/p_text.txt" "$RS" \
      reddit_even_domain_cpp_to_rs
    run_case "$RS"  "-r" "$WORK/cover_even_domain.png" "$WORK/p_text.txt" "$CPP" \
      reddit_even_domain_rs_to_cpp
    EXPECTED_PASSES=$((EXPECTED_PASSES + 2))
  fi

  echo "----"
  echo "passed=$PASSES failed=$FAILS"
  if [[ "$PASSES" -eq "$EXPECTED_PASSES" && "$FAILS" -eq 0 ]]; then
    echo "ALL INTEROP CASES PASSED ($PASSES/$EXPECTED_PASSES)"
  else
    echo "INTEROP FAILURE: expected $EXPECTED_PASSES passes" >&2
    exit 1
  fi
}
