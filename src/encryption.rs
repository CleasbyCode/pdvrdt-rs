...
/// and the salt lives inside the payload region which this key is required to
/// locate. So it is a cheap, salt-free hash of the PIN alone, domain-separated
/// from everything else. It defends position secrecy, not confidentiality -- the
...
pub fn derive_carrier_key_from_pin(pin: &u64) -> Result<u64> {
    ...
    Ok(u64::from_be_bytes(key_bytes))
}
