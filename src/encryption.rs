use crate::common::{require_span_range, span_has_range, PlatformOption};
...
pub const DEFAULT_PDV_SIG_OFFSET: usize = 101;
pub const MASTODON_PDV_SIG_OFFSET: usize = 502;
...
fn generate_recovery_pin() -> u64 {
    let mut pin = 0u64;
    while pin == 0 {
        let mut pin_bytes = [0u8; 8];
        randombytes_into(&mut pin_bytes);
        pin = u64::from_ne_bytes(pin_bytes);
        pin_bytes.zeroize();
    }
    pin
}
...
pub fn encrypt_compressed_file_to_profile(
    ...
    let pin = Zeroizing::new(generate_recovery_pin());
    ...
    zlib_deflate_file(data_file, option, is_compressed_file, |chunk| {
        if chunk.is_empty() {
            return Ok(());
        }
...
pub fn decrypt_data_file(
    ...
    let recovery_pin = Zeroizing::new(get_pin()?);
    let mut salt = Zeroizing::new([0u8; pwhash::SALTBYTES]);
